...
"""Get real-time covid data from arcgis"""
...
raw_json = raw.json()
df = pd.DataFrame(raw_json['features']) #convert data to pandas DataFrame
...
df_final = df_final.dropna(subset=['Last_Update']) #drop rows with NA values in Last_Update column
df_final['Province_State'].fillna(value='', inplace = True) #fill NA values in Province_State with empty strings

df_final['Last_Update']=df_final['Last_Update']/1000
df_final['Last_Update']=df_final['Last_Update'].apply(convertTime) #convert time to human-readable
...
#top 10 countries for number of recovered cases
...
#top 10 countries for number of deaths
...
message += 'Last_Update: ' + df_final['Last_Update'].astype(str)
